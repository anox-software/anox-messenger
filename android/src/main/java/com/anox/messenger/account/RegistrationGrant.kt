 ...
 * credential.
 *
 * The Android client holds this value only in memory or inside the Keystore-wrapped AES-GCM
 * registration session store for the duration of the active transaction. It must never be
 * logged, included in crash reports, or persisted in ordinary preferences, and must never be
 * written to disk as plaintext. [toString] never prints the value to reduce the chance of
 * accidental disclosure through logging or a debugger preview.
 */
