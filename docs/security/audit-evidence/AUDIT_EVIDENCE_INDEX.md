...
**Preservation tasks:** `SECURITY-AUDIT-EVIDENCE-PRESERVATION-001` (`ANOX-EVENT-0045`), `SECURITY-AUDIT-EVIDENCE-PRESERVATION-002` (`ANOX-EVENT-0046`)
**Audit base SHA:** `869b99acac040412a29bbaadc76342070fb2085c` (Security Hardening wave audits); `a79166ab7e65db71ba70e3a427df2ad017dc9225` (`AUDIT-SECURITY-CRYPTO-JNI-001`)
...
`SOURCE_PRESENT = YES` for all six. Sources: the architecture report is the already-canonical repository file (identical bytes also remain at `docs/reports/FINAL_PRE_PRODUCT_SECURITY_ARCHITECTURE_AUDIT.md`); the codebase/consensus/build-supply reports are human-supplied files preserved byte-exact; the Crypto/JNI report is the byte-exact final report body emitted by the authorized specialist audit session (no normalization; see `evidence_hashes.json`).
...
- Next gate: `AUDIT-SECURITY-AUTH-DPOP-001` — **CANDIDATE / NOT_EXECUTED** (then ANDROID/STORAGE, ATTACKCHAIN specialists; then consolidation, remediation sessions, independent retests; only then B-004)

## Validator

`tools/audit/validate_security_audit_evidence_preservation.py` — fail-closed on altered/deleted/incomplete evidence or invalid lifecycle state; extended under PRESERVATION-002 to cover the Crypto/JNI report, candidates, provenance limitation, temp-build evidence, ABI-revision record, and Auth/DPoP not-executed gate. Adversarial tests: `tools/audit/test_security_audit_evidence_preservation.py`.
