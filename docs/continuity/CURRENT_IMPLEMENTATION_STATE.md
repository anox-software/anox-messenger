...
**Date:** 2026-08-30 (PRE-B027-M1R canonical merge lifecycle metadata sync complete)
...
- PRE-B027-M1R permanent canonical merge lifecycle hardening: COMPLETE on `governance/canonical-merge-lifecycle-v1`. Separates `canonical_branch` and `delivery_branch`, distinguishes reviewed delivery tail / merge resolution / post-merge canonical tail, and derives the effective gate from verified lifecycle state. Awaiting `CANONICAL MERGE LIFECYCLE FOCUSED INDEPENDENT REVIEW`.
